...
rule carries the same two recipients the files were encrypted under as every other
creation rule. Divergence between rules is a bug.
...
| ~~`bootstrap/openbao/postgres-openbao.sops.yaml`~~ | **Not held here — see §3.** `mise run update` regenerates this credential in `kubernetes/`, so a copy here would fork from it silently on the next rotation. | — |
...
| `kubernetes/apps/database/postgres/app/passwords.sops.yaml` | Every CNPG role password, including `openbao-postgres-password` — the credential for OpenBao's own storage. **Authoritative**, and hand-maintained since `components/postgres/Update.cs` was retired (docs/postgres-credentials/PLAN.md phase 1) — add one Secret document per new app. Do not copy it into `bootstrap/`. Phases 2–4 of that plan empty this file: `openbao-postgres-password` goes first, replaced by a CNPG client certificate. | Flux → CNPG; OpenBao reads it as a connection URL via `apps/kube-system/openbao/externalsecret.yaml` |
