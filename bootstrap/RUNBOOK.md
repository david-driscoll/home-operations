...
6. **Re-point consumers.** ESO in any surviving cluster needs its `ClusterSecretStore`
   ...
   egress Service — `kubernetes/apps/tailscale-system/services/bootstrap.yaml`, which is
   plain checked-in YAML precisely so this step needs nothing but SOPS and an age key.
   Add a matching grant in `stacks/unifi-network/acl-manager.ts` in the same change, or
   the port is forwarded and the connection then dies in the ACL, which reads from the
   pod as the far end being down.

   > Every **other** egress Service (`proxmox-*`, `pbs-*`, `dns-*`, `dockge-luna`,
   > `dockge-skystar`, …) is generated by `stacks/unifi-network/tailnet-egress.ts` and is
   > NOT editable in git. Only `bootstrap.yaml` (dockge-as, dockge-celestia) and
   > `equestria-kubeproxy.yaml` are hand-maintained — they are the ones the Pulumi
   > operator itself needs in order to run, so they must never depend on Pulumi.
