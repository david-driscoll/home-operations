...
That password lives in `kubernetes/apps/database/postgres/app/passwords.sops.yaml`, which is
SOPS-encrypted in git and decrypts with the same `age.key` as everything here — so it is
already available offline during a cold rebuild, which was the only reason to copy it in the
first place. (It was generated by `components/postgres/Update.cs` until that was retired; it
is hand-maintained now, and phase 2 of docs/postgres-credentials/PLAN.md removes this
particular entry altogether in favour of a CNPG-issued client certificate.)
