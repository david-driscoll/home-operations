...
| 20 low-power tier (**Battery**) | **§4 done; §8 Stages 1–3 all PASS.** Tolerations landed for 38+1 Tier-0/1 workloads (#1073, #1080) after Stage 2 caught `home-assistant` `Pending` on a cordoned cluster; the re-run put it on `pegasus` in 10 s. **Stage 3 run 2026-08-24 on `kerfuffle`** — one hour down, and the number that mattered never moved: non-critical replicas on the trio held at **3** across the 600 s replenishment mark, proving piece 12's `bulk` tagging under real node loss. 44 → 0 degraded in 30 min, CNPG failed over and back, **WoL woke the node in 45 s**. **Stage 4 is the remaining gate** |
