...
## Where the plan actually stands — 2026-08-26
...
| 12 Longhorn critical tier | **landed 2026-08-19** (PR #960). Tags live, `longhorn-critical` exists, the default class is `nodeSelector: bulk`, and the Tier-1 volumes moved onto it (PRs #963/#966). **The Tier-2 backfill that was outstanding has effectively resolved itself** — re-measured 2026-08-26, volumes holding a trio replica went ~100 → 99 → **3**, and all three are disposable VolSync scratch (0.3 GiB). Nobody ran the burndown: §8 Stage 3 forced 93 rebuilds and each was re-placed under the `bulk` constraint it had been ignoring. **9 selector-less volumes remain** — bounded, not a leak ([20](20-low-power-tier.md) §9 item 8) |
| 20 low-power tier (**Battery**) | **§4 done; §8 Stages 1–3 all PASS. Stage 4 prepared 2026-08-24, gated green, and deliberately NOT run.** Tolerations landed for 38+1 Tier-0/1 workloads (#1073, #1080) after Stage 2 caught `home-assistant` `Pending`; the re-run put it on `pegasus` in 10 s. **Stage 3 on `kerfuffle`** held non-critical trio replicas at **3** across the 600 s mark, 44 → 0 degraded in 30 min, CNPG failed over and back, **WoL woke the node in 45 s**. **Stage 4's §6.0 pre-flight passed 9 of 9** at 17:00 on 08-24 — the first fully green run — but entry was held: §6.1 is far bigger than Stage 3 implies (it hibernates CNPG and zeroes OpenBao), `main` was taking ~10 merges/2 h, and the `shining-armor` cordon decision was unmade. **Updated 2026-08-26:** both open questions are now answered — **`shining-armor` stays uncordoned** (so the run gates the outage posture, *not* D6's "trio alone carries Tier 1" premise), and the run is **scheduled for Sunday 2026-08-30, ~09:15**. Two hard constraints were written into 20 in the process: the window must clear **02:00–09:00 ET** at both ends (the downscaler zeroes `garage-operator`, whose fail-closed PVC webhook stalls the Flux resume the exit needs), and **Sunday — not "the weekend" — is the quiet day**: measured over three of each, Saturday is the *busiest* day of the week at ~45 merges/day and Sunday the quietest at ~17. An 08-26 evening attempt was not made; a 26-commit Renovate train at 19:33 had check 3a at 9 degraded `critical` volumes and `technitium` `Pending`. **Stage 4 is the remaining gate** |
| 24 power states (**Low Power**) | **shipped nightly 2026-08-22; first unattended night ran 2026-08-23.** py-kube-downscaler sheds Tier 2 on `--default-downtime=Mon-Sun 02:00-09:00 ${TIMEZONE}` (PR #1046), with Gatus maintenance windows on the 26 affected services (PR #1047). **Workload shed only — no node powers off**, and after the 2026-08-22 reversal none is intended to. The first night fired to the second at both ends, but **29 of 40 workloads bounced back up 10–14 times** — two Flux Kustomizations own `equestria/app-template`, so an hourly `helm upgrade` re-renders `spec.replicas` past the drift-ignore. Fixed here; see 24's "First unattended night" |
| 30 Longhorn media tier | **ABANDONED 2026-08-22, same day it shipped.** PRs #1051/#1053/#1054 built `longhorn-media`, the `low-power`/`low-power-off` node tags and the relocation machinery — **all reverted that evening**. The class is deleted and the three media PVCs are on `longhorn` (bulk), where they always were. **Correction 2026-08-26:** the tags are gone from `talconfig.yaml` but **`low-power` is still live on 5 Longhorn nodes** — `default-node-tags` applies only at node *registration*, so reverting Git never removed it. Verified inert (no StorageClass and **zero** volumes select it), but it is the same reverting-Git-does-not-revert-live-state shape 30 exists to teach. 30 is the post-mortem |
...
**every node still powered.** Battery is [20](20-low-power-tier.md)'s S′: the three
**bare-metal** workers off, Tier 0/1 only, on the Pecron. Correction recorded 2026-08-22 —
...
> ⚠️ **REVERSAL — 2026-08-22 evening.** Earlier that day this section said Low Power would power
> both media workers off "once 30 lands", and described a Longhorn `low-power` node tag that
> marked the five nodes staying up through the window. **All of that was reverted.**
> `fluttershy` and `kerfuffle` **stay powered** overnight and idle instead; the `low-power` and
> `low-power-off` tags are gone from `talconfig.yaml` and the `longhorn-media` class is deleted.
> **Low Power sheds workloads, never nodes.** See [30](30-longhorn-media-tier.md) for what was
> tried, what each shape cost, and why keeping the workers powered won.
...
| [30-longhorn-media-tier.md](30-longhorn-media-tier.md) | *(unfiled)* | **ABANDONED design, kept as a post-mortem.** Was the `longhorn-media` tier — moving the plex/jellyfin/dispatcharr config volumes so both media workers could power off overnight. Records the three shapes tried (`bulk`@4, `critical`/`low-power`@3, `low-power-off`@5→3) and why each failed, and the decision to keep the workers powered and idle instead. Worth reading for the findings that outlived it: Longhorn **never auto-evicts tag-nonconforming replicas** (`replenishReplicas()` counts replicas, never re-checks tags), so a `nodeSelector` patch alone moves nothing; `dataLocality: best-effort` mints permanent unscheduled ghost replicas that nothing alerts on; `kustomize.toolkit.fluxcd.io/force: enabled` on a PVC turns a storage-class edit into **data loss**; measured rebuild throughput was 26–33 MiB/s, ~4× below estimate; a deleted replica replenishes in 15–60 s, not after the 600 s interval; and **three Longhorn volume alerts had never been able to fire** — two are fixed here, one is a still-open gap |
