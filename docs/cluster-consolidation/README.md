...
| 20 low-power tier (**Battery**) | **§4 done; §8 Stages 1–3 all PASS. Stage 4 prepared 2026-08-24, gated green, and deliberately NOT run.** Tolerations landed for 38+1 Tier-0/1 workloads (#1073, #1080) after Stage 2 caught `home-assistant` `Pending`; the re-run put it on `pegasus` in 10 s. **Stage 3 on `kerfuffle`** held non-critical trio replicas at **3** across the 600 s mark, 44 → 0 degraded in 30 min, CNPG failed over and back, **WoL woke the node in 45 s**. **Stage 4's §6.0 pre-flight passed 9 of 9** at 17:00 on 08-24 — the first fully green run — but entry was held: §6.1 is far bigger than Stage 3 implies (it hibernates CNPG and zeroes OpenBao), `main` was taking ~10 merges/2 h, and the `shining-armor` cordon decision was unmade. **Stage 4 is the remaining gate** |
