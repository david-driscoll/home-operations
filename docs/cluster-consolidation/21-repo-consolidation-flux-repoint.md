...
`kubernetes/flux/cluster/ks.yaml` copied with **two** deliberate differences,
both commented in full at their sites in that file:

- `prune: false` on `cluster-meta` and `cluster-apps` — step 2, as specified.
- **`cluster-versions` dropped from the `cluster-apps` child patch's
  `substituteFrom`.** This one was *not* anticipated by this piece and is a
  hard Phase C blocker if missed. That ConfigMap is produced by
  `components/common`'s `configMapGenerator` from a repo-root `versions.env`;
  `home-operations` has neither (its `components/common` dropped the
  generator, and `versions.env` is still in the non-Kubernetes backlog table
  above). The 18 `cluster-versions` ConfigMaps live today are generated by
  `equestria-cluster`'s `-ref` stubs, which stop being read at the exact
  instant of the flip — so the ConfigMap disappears and the reference to it
  breaks in the same reconcile. `substituteFrom` is not optional by default,
  so every child Kustomization would fail with
  `ConfigMap/cluster-versions not found`. Dropping it is verified safe:
  grepping both repos' `kubernetes/` trees for `${KUBERNETES_VERSION}` and
  `${TALOS_VERSION}` returns nothing, and the only real consumer of
  `versions.env` is `.taskfiles/talos/Taskfile.yaml`, which reads the file
  directly. **If `versions.env` is ported later, restore the generator and
  this entry together** — either alone is a silent half-fix.
