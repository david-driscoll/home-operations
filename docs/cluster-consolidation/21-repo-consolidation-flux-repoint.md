...
**Done 2026-08-14** for steps 1–2. `kubernetes/flux/meta/**` copied
byte-identical (verified with `diff -r`; both repos' `.sops.yaml` already list
the same two age recipients, so no re-encryption pass was needed).
`kubernetes/flux/cluster/ks.yaml` copied with exactly one deliberate
difference: `prune: false` on `cluster-meta` and `cluster-apps`, as step 2
specifies, commented in full at its site.

### The `cluster-versions` trap — a Phase C blocker this piece did not anticipate

`cluster-apps`'s child patch injects `cluster-versions` (a ConfigMap) into
every child Kustomization's `postBuild.substituteFrom`. That ConfigMap is
produced by `components/common`'s `configMapGenerator` from a repo-root
`versions.env` — and **`home-operations` had neither**: its `components/common`
had dropped the generator, and `versions.env` was still sitting in the
non-Kubernetes backlog table above.

The 18 `cluster-versions` ConfigMaps that exist live are generated by
`equestria-cluster`'s `-ref` stubs, which stop being read at the *exact*
instant of the flip. So the ConfigMap disappears and the reference to it breaks
in the same reconcile. `substituteFrom` is not optional by default, so every
child Kustomization would have failed with
`ConfigMap/cluster-versions not found` — the whole tree, at once, at the moment
of the flip.

**Resolved by porting the mechanism, not by dropping the reference.** This
commit adds `versions.env` at the repo root and restores the
`configMapGenerator` to `components/common`, keeping `cluster-versions` in all
three places it has to appear.

The first instinct was to drop it instead, on the evidence that nothing
substitutes `${KUBERNETES_VERSION}` or `${TALOS_VERSION}` anywhere in either
repo's `kubernetes/` tree (verified — no matches). **That reading was too
narrow.** These versions drive tuppr's Talos and Kubernetes upgrades. Today
tuppr's CRs in `kubernetes/apps/system-upgrade/upgrades/` carry *literal*
versions with their own `# renovate:` annotations, pointed at the same
datasources (`ghcr.io/siderolabs/installer`, `ghcr.io/siderolabs/kubelet`) as
`versions.env`'s. Nothing would have broken immediately — but the estate would
have been left with no substitution mechanism at all, foreclosing the obvious
consolidation.

**Three sites must move together**, and the failure modes are asymmetric:

| Site | If missing |
|---|---|
| `versions.env` (repo root) | generator fails the build outright |
| `components/common` — generator **and** both `substituteFrom` lists | reference without generator: **every child Kustomization fails**; generator without reference: silently substitutes nothing |
| `kubernetes/flux/cluster/ks.yaml` — the `cluster-apps` child patch | same as above, cluster-wide |

Two gotchas worth carrying forward: `disableNameSuffixHash: true` is required
(the references are by literal name, so a content-hash suffix breaks them), and
because `versions.env` sits outside the component directory, a **local**
`kustomize build` now needs `--load-restrictor LoadRestrictionsNone`.
Flux's kustomize-controller already runs with that setting, which is why
`equestria-cluster` has always needed the flag locally and never in-cluster.

**Follow-up, not done here:** `versions.env` and tuppr's inline literals are now
two independently Renovate-bumped copies of the same two versions. They agree
today (`v1.13.8` / `v1.36.3`). Pointing tuppr's CRs at `${TALOS_VERSION}` /
`${KUBERNETES_VERSION}` collapses them onto one source of truth; until then,
treat a mismatch between the two as a bug.

Parity findings from step 3 worth recording, neither of which blocks the flip:

- `kubernetes/components/**` is no longer byte-identical between the repos, but
  the surviving differences are all comment text plus one trailing newline —
  except `components/common/kustomization.yaml`, which differs *substantively
  and correctly*: `home-operations` carries the `kind: Kustomization` patch
  target that `equestria-cluster` lacks (nested `ks.yaml` files otherwise lose
  their envsubst context) and omits the `cluster-versions` generator. Do not
  "fix" this drift toward equestria.
- `components/volsync-restore` exists only in `equestria-cluster`, **and is
  broken there**: `44b5c0bb6` deleted `replicationdestination.yaml` but left
  the `resources:` entry pointing at it, so the component fails to build if
  anything ever references it. Nothing does in either repo (only prose
  mentions in `components/volsync/AGENTS.md` and three `ks.yaml` comments).
  Deliberately **not** ported — porting it would import a broken component.
  The AGENTS.md restore procedure that references it needs rewriting or the
  component needs rebuilding; tracked as cleanup, not a cutover blocker.
...
| Several namespaces reinstalling simultaneously | **Confirmed live, 2026-08-13**: API-server pressure from many concurrent HelmRelease reinstalls (an unavoidable side effect of the `<namespace>-ref` redirect flip, see [27](27-migration-churn-failure-modes.md)) crash-looped `cilium-operator` on unrelated lease-renewal timeouts, which then silently failed to elect an L2-announcement leader for `network/traefik` — the shared ingress point for nearly every app. Internal cluster traffic stayed healthy the whole time; every external client got `no route to host` for over 30 minutes | `kubectl get lease -n kube-system \| grep l2announce` is missing an entry for a `LoadBalancer` Service that otherwise has a healthy assigned IP; `cilium-dbg statedb dump`'s `l2-announce` table is empty on that Service's backend nodes | A scoped no-op annotation on the affected Service (`kubectl annotate svc <name> -n <ns> l2-nudge=$(date -u +%Y-%m-%dT%H:%M:%SZ) --overwrite`) forces `cilium-operator` to re-evaluate it; a lease appears within seconds. Restarting `cilium-operator` alone was necessary but not sufficient in the observed incident | None once the lease is re-elected. Consider migrating fewer namespaces per sitting, or watching `kubectl get lease -n kube-system \| grep l2announce` after each merge, until this has PrometheusRule coverage (proposed in [27](27-migration-churn-failure-modes.md)) |
...
- [27-migration-churn-failure-modes.md](27-migration-churn-failure-modes.md) — two more incidents from this piece's Phase A: `cilium-operator` silently dropping L2-announcement leader election, and a staged-app Gatus monitoring collision
