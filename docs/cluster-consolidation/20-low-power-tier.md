...
9. **`hard-hat`'s stale talconfig `deviceSelector`** (§6.2) — names a MAC that does not
    ...
    **A second leftover of the same shape, found 2026-08-24 while preparing §8 Stage 3 —
    and this one is fixed.** `hard-hat`, `fluttershy` and `kerfuffle` each still carried
    `vip: {ip: 10.10.206.201}` — the **API server VIP** — in `talos/talconfig.yaml`, left
    behind by [19](19-rotate-equestria-control-planes.md)'s rotation: all three were control
    planes, and the stanza was never removed when they were wiped and rejoined as workers.

    **It was inert, and that was verified rather than assumed.** ARP for `10.10.206.201`
    resolves to `othalla`'s NIC (`e0:51:d8:19:d4:98`), a current control plane — Talos only
    advertises the VIP from a control plane, so a worker declaring it never claims the
    address.

    > ⚠️ **The first attempt to verify this was unsound, which is worth more than the finding
    > itself.** `talosctl … get addresses | grep -c` returned `0`, which reads as "the node
    > does not hold the VIP" and was quoted as such — but `talos/clusterconfig/talosconfig`
    > is **empty** in a fresh worktree (the rendered configs are generated by
    > `talos:genconfig` and are not committed), so `talosctl` had failed and the `0` meant
    > "no output". A count of zero from a command that errored is indistinguishable from a
    > count of zero from a command that worked. ARP was the check that actually answered it.

    Removed from the three workers; the three control-plane entries keep theirs. Recorded
    because "inert today" is not "correct": the stanza is configuration asserting the node
    may serve the Kubernetes API, and nothing in the file said it was vestigial.

    **Not applied.** `talconfig.yaml` is not GitOps — Flux never reads it — so this is inert
    in-cluster until someone runs an apply, and it must be applied **per node**
    (`talos:apply-node`), never with a blanket `mise run talos:apply`, which would carry any
    pending Kubernetes version bump into a tuppr-owned upgrade.
