...
function configureProxmoxAccess(manager: TailscaleAclManager) {
  ...
  manager.setGrant({ src: [tag.ssh], dst: [tag.proxmox], ip: ports.ssh }, { accept: [tag.ssh], deny: testData.knownNormalUsers });
...
function configureDockgeAccess(manager: TailscaleAclManager) {
  ...
  manager.setGrant({ src: [tag.ssh], dst: [tag.dockge], ip: ports.ssh }, { accept: [tag.ssh], deny: testData.knownNormalUsers });
