...
```sh
ws$ kubectl -n stargate-command scale deploy authentik-server authentik-worker --replicas=0
ws$ kubectl -n stargate-command annotate cluster authentik-pg cnpg.io/hibernation=on --overwrite
```

Both are live edits that Flux will revert on its next reconcile of the
`authentik` Kustomization — P5 makes git agree before that matters. Do NOT
`flux suspend` that Kustomization: it owns the CNPG cluster too, and a
suspended Kustomization would never apply P5's hibernation either.
...
2. `kubernetes/apps/stargate-command/authentik/helmrelease.yaml`: `server.replicas: 0`
   and `worker.replicas: 0`.
3. `kubernetes/apps/stargate-command/authentik/resources/postgres-values.yaml`: under
...
ws$ kubectl -n stargate-command get deploy authentik-server     # 0/0
...
failover commit); leave authentik on equestria at zero replicas. Wait for the
cluster to become healthy. It comes back with the pre-failover data, which is about to
be replaced.
...
commit** (items 1 and 2; item 3 went in F1). Flux scales equestria's authentik back up;
...
ws$ kubectl -n stargate-command get deploy authentik-server        # ready
```
